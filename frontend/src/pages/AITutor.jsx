...
import { Send, Bot, Code2, BarChart2, RefreshCw, Copy, Check } from 'lucide-react';
        ...
        if (parts.length > 1) {
          return (
            <p key={i}>
              {parts.map((p, j) => j % 2 === 0
                ? p
                : <code key={j} className="bg-surface px-1.5 py-0.5 rounded text-cyan-400 font-mono text-xs">{p}</code>
              )}
            </p>
          );
        }
...
if (codeMatch) {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      {/* Chat panel */}
      <div className="flex-1 flex flex-col bg-surface2 border-r border-surface">
        {/* Header */}
        <div className="px-4 py-3 border-b border-surface flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-accent to-cyan-500 flex items-center justify-center text-lg">🤖</div>
            <div>
              <div className="text-sm font-semibold">NeuralPath AI Tutor</div>
              <div className="text-xs text-emerald-400">● Online · {usage?.remaining === 'unlimited' ? 'Unlimited queries' : `${usage?.remaining || 0} queries left today`}</div>
            </div>
          </div>
        ...
        {/* Mode selector */}
        <div className="px-4 py-2 border-b border-surface flex gap-1.5 overflow-x-auto">
          {MODES.map(m => (
            <button
              key={m.id}
        ...
        {/* Quick prompts */}
        <div className="px-4 py-2 border-t border-surface flex gap-1.5 overflow-x-auto">
          {QUICK_PROMPTS.map(p => (
            <button
              key={p}
        ...
        <div className="p-4 border-t border-surface">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              rows={1}
              className="flex-1 input resize-none max-h-32 py-2.5"
            ...
            <button
              ...
              className="btn-primary h-[42px] px-3 disabled:opacity-40"
            >
              <Send size={15} />
            </button>
          </div>
      ...
      {/* Code panel */}
      <div className="w-80 flex flex-col border-l border-surface hidden lg:flex">
        ...
        <div className="flex-1 overflow-auto">
          <pre className="code-block rounded-none border-0 text-xs h-full m-0 leading-relaxed">
